...
from dataloader import TestDataset, TrainDataset, TestRewriteDataset, TrainTboxDataset, SingledirectionalOneShotIterator
...
def parse_args(args=None):
    ...
    parser.add_argument('--tbox', action='store_true', help="enable ontology")
    parser.add_argument('--mean_gen', action='store_true', help="mean of distances to generalized queries; otherwise it is sum")
    parser.add_argument('--mean_gen_new', action='store_true',
                        help="mean of distances to generalized queries; This is inside logsigmoid")
    parser.add_argument('--intersec_gen', action='store_true',
                        help="distance to the intersection of the generalized queries; otherwise it is sum")
    parser.add_argument('--filter_neg_examples', action='store_true', help="filter answers of generalizations form being picked as negative examples")
...
def load_data(args, tasks):
    ...
    if args.query_rew:
        test_specializations = pickle.load(open(os.path.join(args.data_path, "test-specializations.pkl"), 'rb'))
        ...
        train_answers = None
        valid_queries = None
        valid_hard_answers = None
        valid_easy_answers = None
    ...
    if args.tbox:
        train_generalizations = pickle.load(open(os.path.join(args.data_path, "train-generalizations.pkl"), 'rb'))
    else:
        train_generalizations = None


    # test if all valid_queries are in both valid_hard and valid_easy
    # queries = set()
    # for q in valid_queries.values():
    #     queries = queries.union(q)
    #
    # for q in queries:
    #     if q not in valid_hard_answers:
    #         print('%s not in valid hard set' %str(q))
    #     if q not in valid_easy_answers:
    #         print('%s not in valid easy set' % str(q))
    #
    # queries = set()
    # for q in test_queries.values():
    #     queries = queries.union(q)
    # for q in queries:
    #     if q not in test_hard_answers:
    #         print('%s not in test hard set' %str(q))
    #     if q not in test_easy_answers:
    #         print('%s not in test easy set' % str(q))

    # test_queries_set = set()
    # for q in test_queries.values():
    #   test_queries_set = test_queries_set.union(q)
    #
    # train_queries_set = set()
    # for q in test_queries.values():
    #   train_queries_set = train_queries_set.union(q)
    #
    # valid_queries_set = set()
    # for q in valid_queries.values():
    #     valid_queries_set = valid_queries_set.union(q)
    #
    # for q in test_queries_set:
    #     if q not in train_queries_set:
    #         print('%s in train set'%str(q))
    #     if q not in valid_queries_set:
    #         print('%s in valid set' % str(q))

    print('---- data check ended')
    ...
    for name in all_tasks:
        ...
        if name not in tasks or evaluate_union != args.evaluate_union:
            ...
            if train_queries is not None and query_structure in train_queries:
                del train_queries[query_structure]
            if valid_queries is not None and query_structure in valid_queries:
                del valid_queries[query_structure]
            if test_queries is not None and query_structure in test_queries:
                del test_queries[query_structure]

    return train_queries, train_answers, valid_queries, valid_hard_answers, valid_easy_answers, test_queries, test_hard_answers, test_easy_answers, train_generalizations, test_specializations
...
def main(args):
    ...
    if args.geo in ['box']:
        if args.mean_gen_new:
            tmp_str = "g-{}-mode-{}-mean_gen_new".format(args.gamma, args.box_mode)
        elif args.mean_gen:
            tmp_str = "g-{}-mode-{}-mean_gen".format(args.gamma, args.box_mode)
        elif args.intersec_gen:
            tmp_str = "g-{}-mode-{}-intersec_gen".format(args.gamma, args.box_mode)
        else:
    ...
    if args.checkpoint_path is not None:
        ...
        args.save_path = os.path.join(args.save_path, tmp_str)

    if not os.path.exists(args.save_path):
        os.makedirs(r'%s' % args.save_path)

    print ("logging to", args.save_path)
    if not args.do_train: # if not training, then create tensorboard files in some tmp location
        writer = SummaryWriter(r'./logs-debug/unused-tb')
    else:
        writer = SummaryWriter(r'%s' % args.save_path)
    set_logger(args)
    ...
    train_queries, train_answers, valid_queries, valid_hard_answers, valid_easy_answers, test_queries, test_hard_answers, test_easy_answers, train_generalizations, test_specializations = load_data(args, tasks)
    ...
    if args.do_train:
        ...
        if args.tbox:
            train_path_iterator = SingledirectionalOneShotIterator(DataLoader(
                TrainTboxDataset(train_path_queries, nentity, nrelation, args.negative_sample_size, train_answers, train_generalizations,args.filter_neg_examples),
                batch_size=args.batch_size,
                shuffle=True,
                num_workers=args.cpu_num,
                collate_fn=TrainTboxDataset.collate_fn
            ))
        else:
            train_path_iterator = SingledirectionalOneShotIterator(DataLoader(
        ...
        if len(train_other_queries) > 0:
            ...
            if args.tbox:
                train_other_iterator = SingledirectionalOneShotIterator(DataLoader(
                    TrainTboxDataset(train_other_queries, nentity, nrelation, args.negative_sample_size, train_answers,train_generalizations,args.filter_neg_examples),
                    batch_size=args.batch_size,
                    shuffle=True,
                    num_workers=args.cpu_num,
                    collate_fn=TrainTboxDataset.collate_fn
    ...
    if args.checkpoint_path is not None:
        ...
        checkpoint = torch.load(os.path.join(args.checkpoint_path, 'checkpoint'),map_location=torch.device('cpu') )
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step % args.valid_steps == 0 and step > 0:
                if args.do_valid:
                    ...
                    # test if each query is in the hard and easy dict
